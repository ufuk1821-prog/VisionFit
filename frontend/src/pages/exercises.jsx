      ...
      <main className="md:ml-64 min-h-screen pt-20 md:pt-10 pb-20 px-4 md:px-section-padding">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-bento-gap max-w-[1400px] mx-auto">
          {activeTab === 'diyagram' ? (
            ...
            <section className="lg:col-span-5 bg-surface-container rounded-xl border border-outline-variant p-6 min-h-[720px]">
              <div className="grid grid-cols-2 gap-3">
                {MUSCLE_GROUPS.map((m) => (
                  <button
                    key={m.key}
                    onClick={() => setSelectedMuscle(m.key)}
                    className={`p-5 rounded-xl border font-label-mono text-label-mono uppercase text-sm text-left transition-all hover:scale-[1.02] ${
